...
RSpec.describe DiscourseArzToolsTopicViewsController do
  fab!(:admin)
  fab!(:user)
  fab!(:admin_api_key) { Fabricate(:api_key, user: admin) }
  fab!(:user_api_key) { Fabricate(:api_key, user: user) }
  ...
  it "buffers an authenticated visitor view for an admin API request" do
    expect(::DiscourseArzTools::TopicViews::Intake).to receive(:accept!).with(
      topic_id: "202277",
  ...
  it "rejects an admin browser session without API authentication" do
    ...
    post path, params: { topic_id: 1, ip: "192.0.2.1" }

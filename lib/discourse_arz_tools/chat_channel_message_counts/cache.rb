...
module ::DiscourseArzTools
  module ChatChannelMessageCounts
    class Cache
      CACHE_VERSION = "v1"
      FRESH_CACHE_KEY = "discourse_arz_tools:chat_channel_message_counts:#{CACHE_VERSION}:fresh"
      STALE_CACHE_KEY = "discourse_arz_tools:chat_channel_message_counts:#{CACHE_VERSION}:stale"
      LOCK_KEY = "discourse_arz_tools:chat_channel_message_counts:#{CACHE_VERSION}:refresh"
      ...
      class << self
        ...
        def refresh!
          ...
          Rails.logger.error(
            "[discourse-arz-tools] failed to refresh chat channel message counts: " \
        ...
        def cache_ttl_seconds
          SiteSetting.discourse_arz_tools_chat_channel_message_counts_cache_ttl_seconds.to_i
        end

        def stale_ttl_seconds
          [
            SiteSetting.discourse_arz_tools_chat_channel_message_counts_stale_ttl_seconds.to_i,
            cache_ttl_seconds,
          ].max
        end
      end
